# -*- coding: utf-8 -*-
"""
Direct database fix for module uninstallation.
This script directly modifies the Odoo database to fix the 'ir.model.function' error.

Usage:
    python3 fix_uninstall.py --db-name odoo --db-user odoo --db-password your_password
"""
import argparse
import logging
import psycopg2
...
def fix_database(db_name, db_user, db_password, db_host='localhost', db_port=5432):
    ...
    _logger.info(f"Starting database fix for {module_name} module")
    ...
    try:
        ...
        if not module_id:
            _logger.error(f"Module {module_name} not found in the database")
            return False
        module_id = module_id[0]
        _logger.info(f"Module ID: {module_id}")
        ...
        # 4. Check if ir_model_function table exists and remove entries if it does
        cursor.execute("""
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_name = 'ir_model_function'
            )
        """)
        if cursor.fetchone()[0]:
            _logger.info("Table ir_model_function exists, removing entries")
            cursor.execute("""
                DELETE FROM ir_model_function 
                WHERE module_id = %s
            """, (module_id,))
        else:
            _logger.info("Table ir_model_function does not exist")
        
        # 5. Set the module state to 'uninstalled' directly
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("The module has been forcibly uninstalled from the database.")
        print("You may need to restart the Odoo server for changes to take effect.")
    else:
        print("\n===== DATABASE FIX FAILED =====")
